...
console.log('✅ Static assets (/uploads, /stamps) configured');
    ...
    return posts.map(post => {
      ...
      if (Array.isArray(p.reactions)) {
        p.reactions = p.reactions.map(r => ({
          ...
          userAvatar: userMap[String(r.userId)]?.avatar || r.userAvatar || null
        }));
      }
      ...
      if (Array.isArray(p.comments)) {
        p.comments = p.comments.map(c => ({
          ...
          userAvatar: userMap[String(c.userId)]?.avatar || c.userAvatar || null
        }));
      }
...
// MOVED TO TOP to avoid conflicts with messy routers below
  ...
  try {
    ...
    if (existingIndex >= 0) {
      if (post.reactions[existingIndex].emoji === emoji) {
        ...
        console.log('   - Changing reaction from', post.reactions[existingIndex].emoji, 'to', emoji);
    ...
    await post.save();
    console.log('   ✅ Post saved successfully. New reactions count:', post.reactions.length);
...
try {
  ...
  console.log('  ✅ /api/posts/comments (router) loaded');
} catch (err) {
...
app.put('/api/groups/:groupId/members/add', async (req, res) => {
  try {
    const { groupId } = req.params;
...
app.put('/api/groups/:groupId/members/remove', async (req, res) => {
  try {
    const { groupId } = req.params;
    const { memberId } = req.body;
    if (!memberId) return res.status(400).json({ success: false, error: 'memberId required' });
...
app.get('/api/users/:userId/sections', async (req, res) => {
  try {
    ...
    const filteredSections = sections.filter(section => {
      ...
      if (section.collaborators && Array.isArray(section.collaborators) && section.collaborators.includes(String(requesterUserId))) {
        return true;
      }
...
app.post('/api/users/:userId/sections', async (req, res) => {
  try {
    ...
    const { name, postIds, coverImage, visibility, collaborators } = req.body;
    ...
    const isOwner = requesterUserId && user.candidates.map(c => String(c)).includes(String(requesterUserId));
    if (!isOwner) {
...
app.put('/api/users/:userId/sections/:sectionId', async (req, res) => {
  try {
    ...
    const requesterUserId = req.body.requesterUserId || req.body.viewerId || req.query.requesterUserId || req.query.viewerId;
    const { name, postIds, coverImage, visibility, collaborators, addPostId, removePostId } = req.body;
    ...
    // AUTHORIZATION CHECK
    const isOwner = requesterUserId && userIdCandidates.map(c => String(c)).includes(String(requesterUserId));
    const isCollaborator = existing.collaborators && Array.isArray(existing.collaborators) && existing.collaborators.includes(String(requesterUserId));
    ...
    if (isOwner) {
      ...
      if (name !== undefined || coverImage !== undefined || visibility !== undefined || collaborators !== undefined) {
