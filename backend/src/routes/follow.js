  ...
  try {
    ...
    try {
      ...
      const followerUser = await User.findOne({ 
        $or: [
          { _id: mongoose.Types.ObjectId.isValid(followerIdCanonical) ? new mongoose.Types.ObjectId(followerIdCanonical) : null },
          { firebaseUid: follower.raw },
          { uid: follower.raw }
        ]
      ...
      notificationQueue.add('follow', {
        userId: followingIdCanonical,
        senderId: followerIdCanonical,
        title: 'New Follower!',
        body: `${senderName} started following you`,
        data: { senderId: followerIdCanonical, type: 'FOLLOW' }
      }).catch(() => {});
    } catch (e) {
    ...
    // Build queries that match ANY of the candidate IDs (firebaseUid, uid, or MongoDB _id)
    const followerQuery = {
      $or: [
        ...follower.candidates.map(c => mongoose.Types.ObjectId.isValid(c) ? { _id: new mongoose.Types.ObjectId(c) } : null).filter(Boolean),
        { firebaseUid: follower.candidates },
        { uid: follower.candidates }
      ]
    };
    
    const followingQuery = {
      $or: [
        ...following.candidates.map(c => mongoose.Types.ObjectId.isValid(c) ? { _id: new mongoose.Types.ObjectId(c) } : null).filter(Boolean),
        { firebaseUid: following.candidates },
        { uid: following.candidates }
      ]
    };

    // Increment following count for follower
    const followerUpdateResult = await User.updateOne(
      followerQuery,
      { $inc: { followingCount: 1 } }
    );
    console.log('[POST /follow] Follower count updated:', followerUpdateResult);

    // Increment followers count for following user
    const followingUpdateResult = await User.updateOne(
      followingQuery,
...
router.delete('/', verifyToken, async (req, res) => {
  try {
    ...
    // Build queries that match ANY of the candidate IDs
    const followerQuery = {
      $or: [
        ...follower.candidates.map(c => mongoose.Types.ObjectId.isValid(c) ? { _id: new mongoose.Types.ObjectId(c) } : null).filter(Boolean),
        { firebaseUid: follower.candidates },
    ...
    await User.updateOne(followerQuery, { $inc: { followingCount: -1 } });

    // Clamp to prevent negative counts (best-effort)
    await User.updateOne(followerQuery, { $max: { followingCount: 0 } });

    // Decrement followers count for following user
    await User.updateOne(followingQuery, { $inc: { followersCount: -1 } });
...
// Check if user is following another user (GET /api/follow/status) — optionalAuth
...
// Get followers of a user with full user details — optionalAuth
...
// Accept follow request (POST /api/follow/request/:requestId/accept) — authenticated
...
// Reject follow request (DELETE /api/follow/request/:requestId) — authenticated
router.delete('/request/:requestId', verifyToken, async (req, res) => {
  try {
    const { requestId } = req.params;
...
// Get pending follow requests for a user — authenticated (own requests only)
...
// Check if follow request exists — optionalAuth
router.get('/request/check', optionalAuth, async (req, res) => {
  try {
...
// Get blocked users list — authenticated (own list only)
...
// Get friends (mutual follows) — optionalAuth
router.get('/users/:userId/friends', optionalAuth, async (req, res) => {
  try {
    const { userId } = req.params;
